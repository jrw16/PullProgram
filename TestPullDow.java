...
for(int t = 1; t < (numOfPanels+1); t++)
{
	...
	// Setting file names
		//file = "11125p" + String.valueOf(t) + inpExt;					// Testing
		//outfile = "TestP" + String.valueOf(t) + outExt;				// Testing
		file = "11125PA3" + inpExt;					// Testing
		outfile = "TestPA3" + outExt;				// Testing
		
	// Array info for pulling names
		int[] pos = {642,642,642,642,642,642,642,642};						// Array of starting positions to pull
		int[] add = {1,1,1,1,1,1,1,1};									// The number past the starting position
		String[] tar = {"1","2","3","4","5","6","7","8"};				// The target for the pull from the data
		
		int[] pos2 = {739,739,739,739,739,802,645,645};						// Array of starting positions to pull second target
		int[] add2 = {2,2,2,2,2,1,1,1};									// The number past the starting position for the second target
		String[] tar2 = {"A3","A4","A5","E1","F1",".","4","5"};			// The second target to pull from the data
		...
		for(int i = 0; i < pos.length; i++)
		{
			...
			if(pulledRecord.equalsIgnoreCase("")) {	
				c1.setFileInputStream(dire, file);
				pulledRecord = c1.pullTarget((titleStartPos-1),2,"Mr",false);	// The 201 pos for Title will change per data file
			}else if(pulledRecord.equalsIgnoreCase("")) {	
					c1.setFileInputStream(dire, file);
					pulledRecord = c1.pullTarget((titleStartPos-1),2,"Ms",false);	// The 201 pos for Title will change per data file
					}
